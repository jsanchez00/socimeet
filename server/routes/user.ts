...
router.post(`${PATH}/update-avatar`, upload.single('profileImg'), async (req: any, res) => {
    ...
    try {
      ...
      fs.realpath(p, (err, pathResolved: string) => {
        if (err) {
          ...
          if (process.env.C_ENV === 'P') fs.unlinkSync(`${pathResolved}/static${userInfo.avatar}`);
          else fs.unlinkSync(`${pathResolved}\\static${userInfo.avatar.split('/').join('\\')}`);
        }
      });
    } catch (e) {
      console.error(e);
